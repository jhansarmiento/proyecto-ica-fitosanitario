...
function NewRoleModal({ isOpen, onClose, onCreate }: NewRoleModalProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      setSaving(true);
      setSubmitError('');
      await onCreate({ rol: roleName.trim(), descripcion: description.trim() });
      onClose();
    } catch (e: any) {
      ...
      if (msg.toLowerCase().includes('duplicado') || msg.toLowerCase().includes('existe') || msg.toLowerCase().includes('unique')) {
        ...
        setSubmitError(msg || 'No fue posible crear el rol. Intenta de nuevo.');
      }
    } finally {
      setSaving(false);
    }
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 px-4 py-8 backdrop-blur-[1px]">
      <div className="w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/5">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-emerald-100">Completa los datos del nuevo rol</p>
              </div>
            </div>
        ...
        <div className="px-6 py-6">
          <h4 className="mb-5 text-2xl font-semibold tracking-tight text-slate-900">Información Rol</h4>
