...
function EditLotModal({ isOpen, lot, onClose, onSuccess }: EditLotModalProps) {
  ...
  const canSave = numero.trim().length >= 3 && Number(areaHa) > 0 && fechaSiembra.trim().length > 0;
      ...
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/5">
        <div className="bg-emerald-900 px-5 py-4 text-white flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-emerald-100">{lot.predio_nombre}</p>
            </div>
          </div>
        ...
        <div className="px-5 py-5 sm:px-6">
          ...
          <div className="mb-6 rounded-xl border border-slate-200 bg-slate-50 p-4">
             <p className="text-sm text-slate-500 uppercase tracking-wide font-semibold mb-1">Cultivo Registrado (No Editable)</p>
             <p className="text-lg font-bold text-slate-800">{lot.especie} <span className="font-normal text-slate-600">— {lot.variedad}</span></p>
          </div>
          ...
          <div className="mt-8 flex justify-end gap-3">
            ...
            <button onClick={handleSave} disabled={!canSave || loading} className="rounded-xl bg-emerald-900 px-6 py-2 text-sm font-semibold text-white hover:bg-emerald-800 disabled:opacity-50 transition">
