...
const users: UserRow[] = [
  { id: 1, identificacion: '1032456789', nombres: 'Laura', apellidos: 'Pineda', rol: 'Administrador', estado: 'Activo' },
  { id: 2, identificacion: '80211455', nombres: 'Carlos', apellidos: 'Ramírez', rol: 'Inspector', estado: 'Activo' },
  { id: 3, identificacion: '1145567890', nombres: 'Diana', apellidos: 'Torres', rol: 'Técnico', estado: 'Inactivo' },
  { id: 4, identificacion: '91234567', nombres: 'Jorge', apellidos: 'Quintero', rol: 'Inspector', estado: 'Activo' },
  { id: 5, identificacion: '1099988877', nombres: 'María', apellidos: 'López', rol: 'Coordinador', estado: 'Bloqueado' },
  { id: 6, identificacion: '1010101010', nombres: 'Andrés', apellidos: 'Castro', rol: 'Técnico', estado: 'Activo' },
];

function badgeClass(status: UserRow['estado']) {
  if (status === 'Activo') return 'bg-emerald-100 text-emerald-700 border-emerald-200';
  if (status === 'Inactivo') return 'bg-amber-100 text-amber-700 border-amber-200';
...
function UsersPage({ onGoHome }: UsersPageProps) {
    ...
    <main className="min-h-screen bg-slate-100 text-slate-900">
      <div className="grid min-h-screen grid-cols-1 xl:grid-cols-[280px_1fr]">
        <aside className="relative flex flex-col overflow-hidden bg-gradient-to-b from-emerald-950 via-emerald-900 to-emerald-950 p-4 text-white">
          ...
          <button
            type="button"
            className="mt-4 flex items-center gap-2 rounded-xl border border-red-300/30 bg-red-500/10 px-3 py-3 text-lg font-bold text-red-300 transition hover:bg-red-500/20"
          >
            <span>↪</span>
            Cerrar Sesión
          </button>
        ...
        <div className="flex flex-col">
          ...
          <section className="flex-1 p-4 sm:p-6">
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="grid grid-cols-[1.2fr_1fr_1fr_0.9fr_0.8fr_0.8fr] border-b border-slate-200 bg-slate-50 px-5 py-3 text-sm font-bold text-slate-600">
                <p>Identificación</p>
                <p>Nombres</p>
                <p>Apellidos</p>
                <p>Rol</p>
              ...
              {filteredUsers.map((row, idx) => (
                <div
                  key={row.id}
                  className={`grid grid-cols-[1.2fr_1fr_1fr_0.9fr_0.8fr_0.8fr] items-center px-5 py-3 text-sm transition hover:bg-emerald-50/40 ${
                ...
                >
                  ...
                  <p>
                    <span className={`inline-flex rounded-full border px-2 py-1 text-xs font-semibold ${badgeClass(row.estado)}`}>
                      {row.estado}
                    </span>
                  </p>
