...
export default function InspectionProcessPage({
  ...
  const [inspectionData, setInspectionData] = useState<
    Record<string, LotInspectionData>
  >({});
  ...
  const mapData = useMemo(() => {
    ...
    const ubicacion = [generalInfo?.municipio, generalInfo?.vereda]
      .filter(Boolean)
      .join(" ");
    const q = encodeURIComponent(
      ubicacion || generalInfo?.nombreLugar || "Colombia",
    );
    return {
      hasCoords: false,
      lat: null,
  ...
  useEffect(() => {
    const fetchContexto = async () => {
      ...
      try {
        setLoading(true);
        const res = await fincaService.getDatosInicioInspeccion(
    ...
    setInspectionData((prev) => {
      ...
      return {
        ...
        [lotId]: {
          ...prev[lotId],
          pestCounts: {
            ...prev[lotId].pestCounts,
            [pestId]: Math.max(0, currentCount + delta),
          },
        },
      };
    });
  ...
  const handleFinalizarInspeccion = async () => {
    try {
      ...
      await fincaService.finalizarInspeccion(solicitud.id_solicitud, {
        lotesInspeccionados,
      });
    ...
    return (
      ...
      >
        <div className="p-10 text-center text-slate-500 animate-pulse">
          Cargando contexto de inspección...
        </div>
      </DashboardLayout>
    );
  }
  ...
  return (
    <DashboardLayout
      ...
      onLogout={onLogout}
    >
      <section className="w-full space-y-4 px-2 pb-4 sm:space-y-5 sm:px-3 md:px-4 lg:px-6">
        <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-2 border-b border-slate-100 bg-linear-to-r from-[#015c4b] to-[#0b6b57] px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div>
              ...
              <h2 className="mt-2 break-words text-xl font-black tracking-tight text-white sm:text-2xl lg:text-3xl">
                {generalInfo.nombreLugar}
              </h2>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 p-3 sm:grid-cols-2 sm:gap-4 sm:p-4 lg:grid-cols-3">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4">
              ...
              <h3 className="mt-1 wrap-break-word text-base font-black text-slate-900 sm:text-lg">
                {generalInfo.coordenadas || mapData.label}
              </h3>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4">
              <p className="text-[11px] font-bold uppercase tracking-wide text-slate-500">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 sm:col-span-2 xl:col-span-1">
              ...
              <h3 className="mt-1 wrap-break-word text-base font-black text-slate-900 sm:text-lg">
                {generalInfo.municipio} - Vda. {generalInfo.vereda}
              </h3>
            </div>
            <a
  ...
  "
            >
              ...
              <div className="absolute left-0 top-0 h-1 w-full bg-linear-to-r from-emerald-500 to-emerald-300"></div>
      ...
      "
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 sm:col-span-2 xl:col-span-1">
              ...
              <h3 className="mt-1 wrap-break-word text-base font-black text-slate-900 sm:text-lg">
                {generalInfo.productorNombre}
              </h3>
              <p className="text-sm font-medium text-slate-500">
                Tel: {generalInfo.productorTelefono}
              </p>
            </div>
          </div>
        </article>

        <div className="mb-2 mt-4 flex flex-col gap-1 sm:mb-4 sm:mt-6 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="text-2xl font-bold text-slate-900">
            Lotes a Inspeccionar
          </h2>
          <p className="text-sm font-semibold text-slate-500">
            {Object.values(inspectionData).filter((d) => d.isCompleted).length}{" "}
            de {lots.length} listos
          </p>
        </div>
          ...
          {lots.map((lot) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div className="flex w-full min-w-0 items-center gap-3 sm:w-auto sm:gap-4">
                    ...
                    <div>
                      ...
                      <p className="wrap-break-word text-sm font-medium text-slate-600">
                        {lot.cultivo}
                      </p>
                      <p className="mt-1 text-xs text-slate-500">
                        Siembra: {lot.fechaSiembra} • {lot.areaHa} ha
                      </p>
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div className="border-t border-slate-100 bg-slate-50/50 p-3 sm:p-4 md:p-5">
                    <div className="grid gap-4 md:grid-cols-3 md:gap-6">
                      <div className="rounded-xl border border-blue-200 bg-white p-5 shadow-sm">
                        <h3 className="mb-4 text-sm font-bold uppercase tracking-wide text-blue-600">
                          Plantas Totales (Conteo Físico)
                        </h3>
                        <div className="flex items-center justify-between">
                          <button
                            onClick={() =>
                          ...
                          <div className="text-center">
                            <p className="text-3xl font-black text-blue-600 sm:text-5xl">
                              {data.totalPlants}
                            </p>
                            <p className="text-xs font-medium text-slate-400">
                              Encontradas en el lote
                            </p>
                          </div>
                          <button
                            onClick={() =>
                      ...
                      <div className="space-y-4 md:col-span-2">
                        ...
                        <label className="block">
                          <span className="text-sm font-bold text-slate-700">
                            Observaciones Específicas del Lote
                          </span>
                          <textarea
                    ...
                    <div className="mt-4 border-t border-slate-200 pt-4 sm:mt-6 sm:pt-5">
                      <h3 className="mb-4 text-sm font-bold uppercase tracking-wide text-slate-700">
                        Registro de Plagas Específicas
                      </h3>
                      <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
                        {plagasDelLote.map((pest) => (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                onClick={() =>
                                  updatePestCount(lot.id, pest.id, -1)
                                }
                                className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-200"
                              >
                                <Minus size={14} />
                              </button>
                              <p className="w-8 text-center font-bold text-slate-800">
                                {data.pestCounts[pest.id] || 0}
                              </p>
                              <button
                                onClick={() =>
                                  updatePestCount(lot.id, pest.id, 1)
                        ...
                        {plagasDelLote.length === 0 && (
                          <p className="text-sm italic text-slate-500">
                            No hay plagas registradas en el catálogo para esta
                            especie.
                          </p>
                        )}
                      </div>
        ...
        <div className="mt-6 flex flex-col items-stretch justify-between gap-3 border-t border-slate-200 pb-6 pt-4 sm:mt-8 sm:pb-10 sm:pt-6 sm:flex-row sm:items-center sm:gap-4">
          <button
            type="button"
