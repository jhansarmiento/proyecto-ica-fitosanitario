...
type InspectionProcessPageProps = {
  ...
  solicitud?: any; // Aquí puedes definir un tipo más específico basado en la estructura de tus solicitudes
  onGoHome?: () => void;
  onGoUsers?: () => void;
  onGoRoles?: () => void;
...
export default function InspectionProcessPage({
  ...
  const [lots, setLots] = useState<LotItem[]>([]); // 🌟 Empiezan vacíos
  const [plagasCat, setPlagasCat] = useState<PestItem[]>([]); // 🌟 Catálogo de plagas dinámico
  ...
  const [pestCounts, setPestCounts] = useState<Record<string, number>>({
    "P-1": 0,
    "P-2": 0,
  });
  ...
  useEffect(() => {
    const fetchContexto = async () => {
      ...
      try {
        setLoading(true);
        // Asegúrate de crear este método GET en finca.service.ts apuntando a /solicitudes/:id/iniciar-inspeccion
        const res = await fincaService.getDatosInicioInspeccion(
          solicitud.id_solicitud,
        );

        setLugarInfo(res.data.productor);
        setLots(res.data.lotes);
        setPlagasCat(res.data.plagas);

        // Seleccionamos el primer lote automáticamente si existe
        if (res.data.lotes.length > 0) {
          setSelectedLotId(res.data.lotes[0].id);
        }
      } catch (error) {
  ...
  const infestationPercent = selectedLot
    ? ((affectedPlants / selectedLot.plantas) * 100).toFixed(2)
    : "0.00";
  ...
  const markLotInspected = (lotId: string) => {
    setLots((prev) =>
      prev.map((lot) =>
        lot.id === lotId ? { ...lot, estado: "Inspeccionado" } : lot,
      ),
    );
    ...
    return (
      ...
      >
        <div className="p-10 text-center text-slate-500 animate-pulse">
          Cargando contexto del lugar de producción...
        </div>
      </DashboardLayout>
    );
  }
        ...
        {step === 1 ? (
          <section className="space-y-4">
            <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              {/* HEADER */}
              <div className="border-b border-slate-100 bg-linear-to-r from-[#015c4b] to-[#0b6b57] px-6 py-4">
    ...
    "
            ...
            <article className="rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="flex items-center justify-between px-5 py-5">
                <button
                  type="button"
                  onClick={() =>
                    setAffectedPlants((prev) => Math.max(0, prev - 1))
                  }
      ...
      "
          ...
          "
                      ...
                      <div className="min-w-17.5 rounded-xl bg-slate-50 px-4 py-2 text-center">
            ...
            <article className="rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="grid gap-4 px-5 py-5 md:grid-cols-2">
                ...
                <div className="rounded-xl border border-rose-200 bg-rose-50 p-4">
                  ...
                  <p className="mt-1 text-4xl font-bold text-rose-600">
                    {affectedPlants}
                  </p>
                </div>
                <div className="rounded-xl border border-rose-200 bg-rose-50 p-4">
